...
Lorentz transformation in y-direction yields that $\rho'=0$ on the vertical sides of the loop. Thus;
...
\Delta \int_A^B\vec x'\rho(\vec r')\,dx'
+\Delta \int_D^C(\vec x'+\vec b') \rho(\vec r')\,dx'
+\Delta' \int_A^D\vec y'0 \,dy'
+\Delta' \int_B^C(\vec y'+\vec a') 0 \,dy' \\
&= \gamma\frac{v}{c^2}  [
\Delta \int_A^B\vec x' j \,dx'
+\Delta \int_D^C (\vec x'+b') (-j)\,dx' ] \\
&= \gamma\frac{v}{c^2} \Delta   (-\vec ba'j )=-\frac{1}{c^2}   (vaj \Delta \vec b )=-\frac{1}{c^2}   (v Ia \vec b )=-\frac{1}{c^2}   (\vec v \cdot I \vec a) \vec b  \\
\end{align*}
Using that the triple product rule: $(\vec a \times \vec b)\times \vec c=-(\vec c \cdot \vec b)\vec a + (\vec c \cdot \vec a) \vec b$, and the fact that $\vec v \cdot \vec b=0$ yields
$\vec m=\frac{1}{c^2} \vec m \times \vec v$
